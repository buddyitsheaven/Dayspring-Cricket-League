...
fixtures.each_with_index do |match_attrs, index|
  ...
  [match.team_one, match.team_two].each do |team_name|
    winner_question.options.create!(label: team_name)
    toss_question.options.create!(label: team_name)
  end

  next unless index.zero?

  winner_question.update!(correct_option: winner_question.options.first)
  toss_question.update!(correct_option: toss_question.options.last)

  viewer.predictions.create!(prediction_question: winner_question, prediction_option: winner_question.options.first)
  viewer.predictions.create!(prediction_question: toss_question, prediction_option: toss_question.options.first)
end
